...
df<-scale(a)
b<-read.csv("p1.csv",header = T,row.names=1)
c<-read.csv("p2.csv",header = T,row.names=1)
...
library(ggtree)
library(ggplot2)
data<-read.csv("AVG1.csv",header = T,row.names=NULL)
df <- scale(data[,2:6])
...
library(ggtree)
library(ggplot2)
a1<-read.csv("AVG1.csv",header = T,row.names=NULL)
a<-read.csv("AVG1.csv",header = T,row.names=1)
df<-scale(a)
b<-t(df)
...
library(MASS)
data<-read.csv("Stepwise.csv",header = T,row.names=NULL) 
